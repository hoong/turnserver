 ...
 */
#define SOFTWARE_DESCRIPTION "TurnServer 0.6"
    ...
    socklen_t saddr_size, struct tls_peer* speer, unsigned char* key)
{
  ...
  switch(error)
  {
    case 400: /* Bad request */
      ...
      break;
    case 402: /* Authentication failed */ /* 91u extra */
      hdr = turn_error_response_402(method, id, &iov[index], &index);
      break;
    case 403: /* Forbidden */
    ...
    struct tls_peer* speer)
{
  ...
  /* Check Session available */ /* 91u extra */
  if (message->user_id && message->session_id)
  {
	uint64_t uid = ntohll(message->user_id->turn_attr_id);
	if (200 != oap_check_session((const char*)message->session_id->turn_attr_session,ntohs(message->session_id->turn_attr_len)))
	{
		debug(DBG_ATTR,"Session Authenticated failed.\n");
		return -1;
	}
  }
  else
  {
	  debug(DBG_ATTR,"Sessionid or uid missed.\n");
      turnserver_send_error(transport_protocol, sock, method,
          message->msg->turn_msg_id, 402, saddr, saddr_size, speer,
          account->key);
      return -1;
  };
...
int main(int argc, char** argv)
{
  ...
  debug(DBG_ATTR, "Run with uid_real=%u gid_real=%u uid_eff=%u gid_eff=%u\n",
	...
	/*91u Extra*/
  if (oap_init(turnserver_cfg_oap_server(),turnserver_cfg_inside_passport()) != 0)
  {
	  debug(DBG_ATTR,"oap init error\n");
	  g_run = 0;
  }
